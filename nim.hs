...
putBoard' :: Board -> IO ()
putBoard' board = recurse (zip [1..] board)
 where
   recurse [x] = putRow (fst x) (snd x)
   recurse (x:xs) = do putRow (fst x) (snd x)
                       recurse xs
...
nim :: IO ()
nim = play initial 1
-- Exercises 10.2 : Using recursion, define a version of putBoard :: Board -> IO () that displays nim boards of any size, rather than being specific to boards with just five rows of stars
-- nim = play (initial' 6) 1